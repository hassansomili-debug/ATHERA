...
## ١) تركيبُ الموجّه في `main.py` — **مطلوب، وبدونه لا تعمل الميزة**

الملفّ: `apps/api/athera_api/main.py`

**المطلوب** — سطرا استيرادٍ وتركيب، بلا شيءٍ آخر:
...
**ما الذي يسقط بدونه**: كلّ شيء. الموجّه `apps/api/athera_api/routers/project_management.py`
مكتوبٌ ومُختبَر، ولا يبلغه طلبُ HTTP واحد حتى يُضاف السطران — فلا لوحةَ
مشروع، ولا مهامّ، ولا مَعالم، ولا معاينةَ إتلاف، ولا سلّة. والشاشات الأربع
في `apps/web` ستُصيَّر وتعرض «تعذّر تحميل هذه الشاشة» في الإنتاج.
...
ولم يُمسّ `main.py` ولا `ci.yml` — وهما موضوع الطلبين ١ و٢ أعلاه.
