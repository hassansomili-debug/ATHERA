...
منصة ذكاء اصطناعي للبحث والنشر العلمي، تحوّل الأفكار والرسائل العلمية والمراجع والبيانات إلى أوراق علمية موثقة وجاهزة للتقديم للمجلات.

المدخلات أربعة — فكرة · رسالة علمية · مجموعة أدبيات · مجموعة بيانات — والمخرَج واحد: مخطوطة مؤصَّلة في الأدلة **جاهزة للتقديم**. جاهزة للتقديم لا تعني نشرًا مضمونًا ولا قبولًا متوقعًا؛ ولا يملك النظام حقلًا لاحتمال القبول.
...
|---|---|---|
| **منطق خالص** | التفكيك، الحواجز، الخيط الذهبي، التداخل، المجلات، التحليل، الاتجاهات | **مُشغَّل** |
...
- [ADR-0001 المكدس المعتمد](docs/adr/ADR-0001-approved-stack.md) · [ADR-0002 العزل](docs/adr/ADR-0002-multi-tenancy-isolation.md) · [ADR-0003 بوابة المزود](docs/adr/ADR-0003-model-provider-gateway.md) · [ADR-0004 التدقيق](docs/adr/ADR-0004-audit-provenance.md) · [**ADR-0005 إعادة التموضع**](docs/adr/ADR-0005-v2-repositioning.md) · [**ADR-0006 أبعاد التقييم**](docs/adr/ADR-0006-publication-scoring-dimensions.md)
...
لا يستدعي نموذجًا في الإنتاج (`MODEL_PROVIDER=null`) · لا يحوّل مخرجات نموذج إلى ذاكرة موثقة · لا يقبل حقيقة باقتباس غير موجود في مصدرها · لا يعتمد حقيقة بلا قرار إنسان · لا يعرض مخرَجًا خالف حاجز نزاهة · لا يملك أداة تعدّل بيانات خامًا أو تبتّ في اعتماد · لا يعرض نسبة جاهزية تخفي شرطًا حاجبًا · لا يخلط الإسقاط بالإنجاز · لا يولّد مرجعًا لادعاء بلا دليل · لا يقتطف نصًا من مصدر لم يُتَح نصه · لا يستشهد بمصدر مسحوب بلا إقرار وسياق · لا يفتح بوابة بروتوكول بعيب اتساق حاجب · لا يعيد درجة اتساق مجردة من أسبابها · لا يتقدم بفرصة نشر بلا اعتماد الحقوق والتأليف · لا يسند تأليفًا لغير إنسان أو جهة · لا يحوّل فرصتين متداخلتين بلا حسم بشري · لا يقدّر احتمال قبول ولا يملك حقلًا له · لا يعتمد مجلة بفهرسة لم يُعَد التحقق منها · لا يعدّل نسخة مخطوطة معتمدة بمراجعة آلية · لا يعدّل بيانات خامًا · لا يحلّل بيانات غير مجمَّدة · لا ينتج نتيجة بلا تشغيلة · لا يدمج التفسير الإحصائي بالنظري بالإداري · لا يحتسب مخرَج نموذج دليلًا على اتجاه · لا يخلط قوة الاتجاه بقابلية النشر · لا يقدّم ورقة خارجيًا بلا فعل بشري أو تفويض قابل للسحب · لا يسمح لواجهة المتصفح بالوصول إلى مزود · لا يغلق بوابة اعتماد بمهلة زمنية · لا يعدّل سجل تدقيق.
