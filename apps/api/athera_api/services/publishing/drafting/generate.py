...
INSTRUCTION: Final = (
    ...
    "٧. **اقرأ `statistical_evidence` قبل أن تكتب حكمًا إحصائيًّا.** هو يقول "
    "ما تملكه الأدلة بالضبط. فإن كان `significance_claim_allowed` خطأً: لا "
    "تقل «دالّ إحصائيًّا» ولا «عند مستوى كذا» ولا «p < كذا»، **ولا تشتقّ "
    "الدلالة من قيمة (ت) ودرجات حريتها ولا من أي رقم آخر**. وغيابها واقعةٌ "
    "علمية تُقال في `missing_evidence`، لا نقصٌ تُكمله. واكتب ما تسنده "
    "الأدلة: الفرق الملحوظ في المتوسطات، وحجم الأثر المسجَّل إن وُجد.\n"
    "٧ب. ولا تصف حجم الأثر بـ«صغير» أو «متوسط» أو «كبير»: تلك عتباتٌ "
    "اصطلاحية لا ترد في الأدلة المرفقة. اذكر القيمة كما هي.\n"
    "٧ج. وإن وُجدت فرضية: افصل **الاتجاه** عن **الدلالة**. تقول «جاءت "
    "النتيجة في الاتجاه الذي افترضته» إن سنده دليل؛ ولا تقول «قُبلت» ولا "
    "«رُفضت» بلا قيمة دلالة مسجَّلة.\n"
    "٨. وإن لم تجد ما يكفي لكتابة القسم، اترك `section_text_ar` فارغًا واذكر "
    "النواقص. القسم الفارغ بقائمة نواقصه أصدق من فقرة معقولة.\n"
    "٩. `origin` يقول أصل كل ادعاء: `fact` لما يسنده دليل، و`inference` "
    "لاستنتاج تبيّن أساسه، و`proposal` لصياغة مقترحة لا تُعرض حقيقة.\n"
    "١٠. لا تكتب معرّفًا لم يرد في القائمة المرفقة."
)
...
SECTION_RULES: Final[dict[str, str]] = {
    "discussion": (
        "\n\nوهذا قسم **المناقشة**، وموضعه بين ما لوحظ وما يُستنتج:\n"
        "٩. فسّر نتائج هذه الدراسة وحدها. ولا تقل «تتفق مع الدراسات "
        "السابقة» ولا «تخالفها» ولا «تؤكد جدّتها» — سجلّ الأدبيات مغلق، "
        "وموضع ذلك `missing_evidence` بوصفه «بانتظار البحث العلمي».\n"
        "١٠. **نقصُ قيمة الدلالة لا يمنع المناقشة.** اكتب ما تسنده الأدلة: "
        "أي المجموعتين كان متوسطها أعلى، وما حجم الأثر المسجَّل. ثم قل "
        "صراحةً إن الحكم على الدلالة الإحصائية غير متاح بالأدلة الحالية.\n"
        "١١. وكل استنتاج تكتبه ضع `origin` له `inference`، وأرفق في "
        "`memory_ids` أو `analysis_output_ids` الأساسَ الذي بُني عليه. "
        "استنتاجٌ بلا أساسٍ معروض ليس استنتاجًا بل رأيًا.\n"
    ),
    "results": (
...
def build_prompt(context) -> str:
    ...
    payload = {
        ...
        # **ما تملكه الأدلة وما لا تملكه — مقروءًا لا مستنتَجًا** (§4).
        # فالنموذج كان يرى `t` و`df` بلا قيمة دلالة، فيحسبها بنفسه ويكتب
        # «دالّ إحصائيًّا»، فيُرفض في كل محاولة. ولم يكن أحدٌ أخبره بما
        # **لا** يملك.
        "statistical_evidence": context.statistics.as_model_view(),
    }
