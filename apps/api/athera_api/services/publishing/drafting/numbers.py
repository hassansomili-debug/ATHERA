...
# كل حركات العربية — تُتخطّى حيثما وقعت، فالتشكيل لا يغيّر الحكم.
_DIACRITICS = r"[\u064B-\u0652\u0670]*"
_SIGNIFICANCE_TAIL = r"(?:\s*عند\s+مستوى(?:\s+الدلالة)?\s*[\d.,]*)?"
SIGNIFICANCE_CLAIMS: Final[tuple[re.Pattern[str], ...]] = (
    # **والتشكيل يُتخطّى لا يُعدّ.** كانت الحركات مسموحًا بها بعد «إحصائي»
    # وحدها، فكان «الفرق **دالٌّ** إحصائيًّا» يمرّ بلا كشف: التنوين والشدّة
    # بعد «دال» يقطعان النمط. والباحث يشكّل أو لا يشكّل، والحكم واحد.
    re.compile(r"دال" + _DIACRITICS + r"(?:ة|ا)?" + _DIACRITICS
               + r"\s+إحصائي" + _DIACRITICS + r"(?:ية|ة|ا)?" + _DIACRITICS
               + _SIGNIFICANCE_TAIL),
