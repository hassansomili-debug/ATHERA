...
`ProjectInvitation` القائمة، والدعوةُ تُقبل بيد صاحبها.

**و«مدعوّ» محجوزةٌ لـRC-T1C ولا طريقَ إليها هنا.** فالدعوةُ الصحيحة دعوةٌ
**لصاحب هذا التطبيق بعينه**، والدعواتُ اليوم محلّيّةُ المستأجر
والتطبيقاتُ عابرةٌ له — فالربطُ الصحيح تصميمُ خدمةٍ لا سطرٌ في مُشغِّل.
و`invitation_id` يبقى بنيةً آمنةً لا بابًا: لا انتقالَ يستعمله، ولا
عضويّةَ ولا وصولًا يمنحه.

والعضويّةُ ليست تأليفًا، ولا أدوارَ CRediT تُكتب من هنا.

## عقدُ التحويل الذي يُغلقه RC-T1C

ويُكتب هنا لأنّ من يبنيه سيقرأ هذه الوحدة، ولأنّ «محجوزٌ لمرحلةٍ قادمة»
بلا شرطٍ مكتوب دعوةٌ إلى أن يُفتح البابُ بأقلّ ما يُظنّ كافيًا:

  ١ يُرشَّح المتقدّم، فتُقرأ **هويّتُه المصادقة** من صفّه لا من بريدٍ
    يُكتب بيد.
  ٢ وتُصدر `ProjectInvitation` **لذلك الحساب بعينه** — وهو ما يستوجب
    تصميمًا: الدعواتُ اليوم تبحث عن الحساب داخل مستأجر البحث،
    والمتقدّمُ قد يكون خارجه.
  ٣ ويختار المديرُ دورًا من مفردات `ProjectMember` القائمة وصلاحياتٍ
    **صريحة** — ولا دورَ يُشتقّ من وسمِ التعاون في الإعلان.
  ٤ ويُحفظ الربطُ (`invitation_id`)، **ولا تصير الحالُ «مدعوّ» إلّا بعد
    نجاح الربطِ الدقيق**.
  ٥ ويقبل المرشَّحُ الدعوةَ **بيده**، وحينها وحدها تُنشأ العضويّة.

ولا إدراجَ مباشرًا في `ProjectMember` من الاستقطاب، ولا تأليفَ تلقائيًّا،
ولا أدوارَ CRediT. و«مساعدُ بحث» وسمُ إعلانٍ لا دورَ عضوية.
...
#: حالاتُ التطبيق.
#:
#: و`invited` **كانت محجوزةً في RC-T1B، وفُتحت في RC-T1C** بربطٍ دقيق.
#:
#: فالدعوةُ الصحيحة تستوجب `ProjectInvitation` **لصاحب هذا التطبيق
#: بعينه**، و`ProjectInvitation` اليوم محلّيّةُ المستأجر: إصدارُها يبحث
#: عن الحساب داخل مستأجر البحث. وفرصُ البحث عابرةٌ للمستأجرين قصدًا.
#: فربطُ المتقدّم بدعوةٍ عبر المستأجرين تصميمُ خدمةٍ في RC-T1C، ولا
#: يُوسَّع سلوكُ الدعوات المحلّيّ خِلسةً في دفعةٍ أمنية.
#:
#: فبقيت المفردةُ في المخطَّط — لا مخطَّطَ يُهاجر لاحقًا — **ولا طريقَ
#: إليها**: `MANAGER_TRANSITIONS` لا تحملها هدفًا، والصفُّ يُولد في
#: الانتظار، وصاحبُ التطبيق لا يملك إلّا الانسحاب.
...
#: يتقدّم لفرصةٍ أخرى. أمّا `invited` فقائمٌ **متى وُجد**: الدعوةُ أُرسلت
#: ولم تُحسم. **ولا صفَّ يبلغها في RC-T1B**، فوجودُها هنا يحفظ الفهرسَ
#: الجزئيَّ من هجرةٍ ثانيةٍ يوم تُفتح.
...
#: ومرآتُها هنا.
#:
#: **و«مدعوّ» فُتحت في RC-T1C** — وشرطُها ربطٌ دقيق لا مجرَّدُ انتقال:
#: دعوةٌ حيّةٌ في بحثِ الفرصة نفسِه، **مُوجَّهةٌ إلى صاحب هذا التطبيق
#: بعينه**. ويُثبت ذلك مُشغِّلٌ على القاعدة (الترحيل 0035)، لا شيفرةُ
#: موجّه.
#:
...
#:
#: ويُشتقُّ من `ACTIVE_APPLICATION_STATES` فيحمل `invited → withdrawn`
#: تبعًا — **وهو غيرُ قابلٍ للبلوغ في RC-T1B** لأنّ لا صفَّ يصل «مدعوّ»
#: أصلًا. والاشتقاقُ أصدقُ من قائمةٍ تُكتب بيد: يوم تُفتح «مدعوّ» يكون
#: الانسحابُ منها قائمًا بلا تعديل.
