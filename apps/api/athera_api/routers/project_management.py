...
تركيبُ الموجّهات صريحٌ في `main.py`، وهو ملفُّ المُكامِل وحده. فالسطران
مطلوبان في `docs/integration/track-b-requests.md`:

    from .routers import project_management as project_management_router
    app.include_router(project_management_router.router)

ويُقال هذا صراحةً لا يُترك ليُكتشف: من قرأ الملف ورأى النقاط قد يظنّ
الميزة قائمة، وهي لا يبلغها طلبُ HTTP حتى يُضاف السطران.
"""
