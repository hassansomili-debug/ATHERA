...
class ReviewResponse(BaseModel):
    thesis_id: uuid.UUID
    #: عنوانُ الرسالة كما استُخرج — و`None` إن لم يُستخرَج بعد.
    #:
    #: **والباحثُ يراجع رسالةً بعينها.** كانت الشاشةُ تقول «مراجعة أدلة
    #: الرسالة» ولا تقول **أيَّ** رسالة، ومن رفع ثلاثًا لا يعرف أيَّها
    #: يقرأ. وإن لم يُستخرَج عنوانٌ بعد فاسمُ الملفّ بديلٌ صريح — لا فراغ.
    thesis_title: str | None = None
    #: اسمُ الملفّ المرفوع — بديلُ العنوان حين لا عنوان.
    source_filename: str | None = None
    sections: list[SectionGroup]
    #: كلُّ ما يُعرض — بما فيه بياناتُ الملفّ النظامية التي لا تُراجَع.
    total: int
    #: **إجماليُّ المراجعة: ما يقبل قرارًا وحده.**
    #:
    #: وكان العدُّ يقوم على `len(FIELD_CATALOGUE)`، وفيه حقولٌ حتميّة
    #: (`page_count`, `source_filename`) يقرؤها الاستخراجُ من بيانات الملفّ
    #: لا من متنه — فلا اقتباسَ لها، و`approve_candidate` تشترط التأصيل في
    #: النصّ. فكانت تُحسب «بانتظار مراجعتك» ولا سبيلَ إلى مراجعتها أبدًا:
    #: عدّادٌ لا يبلغ صفرَه، وباحثٌ يُطلب منه ما لا يمكن فعله.
    #:
    #: فالإجماليُّ هنا = المعتمَد + المرفوض + «لا أعرف» + المنتظِر،
    #: على الحقول القابلة للقرار (`decidable`) وحدها.
