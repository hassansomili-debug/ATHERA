...
class JourneyResponse(BaseModel):
    ...
    #:
    #: و**كلُّ ما ستحتاجه الرحلةُ قبل أن تنتهي** — لا ما يمنع الآن.
    blocking_reasons: list[str] = []
    #: **ما يمنع الخطوةَ التالية بعينها** — يحسبه الخادمُ ولا تشتقّه الشاشة.
    #:
    #: **وكان يُحسب ثمّ يسقط قبل أن يصل.** `journey.view` تعيده في قاموسها،
    #: والموجّه يبني `JourneyResponse(**view)` — و`extra="ignore"` هي
    #: الافتراضُ في Pydantic، فيُطرح المفتاحُ الذي لا يُعلنه النموذجُ
    #: **بلا خطأٍ ولا تحذير**. فتقرأ الشاشةُ `undefined` دائمًا، وتبقى
    #: قائمةُ «ما يمنع الآن» فارغةً أبدًا: إذنٌ غائبٌ يمنع الخيطَ الذهبيّ،
    #: والزرُّ مطفأٌ بلا سببٍ مكتوب — وهو الطريقُ المسدود في صورةٍ أهدأ.
    #:
    #: **وحقلٌ يُحسب ولا يُعلَن أسوأُ من حقلٍ لم يُكتب**: الخادمُ يعمل،
    #: والشاشةُ تقرأ، والعقدُ بينهما يبتلع الجواب بصمت.
    current_blocking_reasons: list[str] = Field(default_factory=list)
