...
async def test_over_http_a_completed_empty_scan_claims_nothing_about_the_world(two_tenants):
    ...
    # وهذه هي دعوى هذا الفحص، وهي قائمة.
    assert "لا فرص" not in reason

    # **وإعادةُ المحاولة متاحة — وهذا عكسُ قرارٍ سابق، بسببه المكتوب.**
    #
    # كان هنا `can_mine is False` وحجّتُه «لا زرَّ يَعِد بنتيجةٍ أخرى من
    # المُدخل نفسه». وقد نُقضت تلك الحجّةُ صراحةً في `card_actions`: كانت
    # صحيحةً يومَ كان التكوينُ ثابتًا، ثمّ صار المُدخل نفسُه يُنتج مقترحًا
    # محافظًا (`_fallback_draft`) لم يكن يُنتجه. فرسالةٌ خُتمت بصفر قبل
    # الإصلاح تُنتج اليوم فرصةً عند إعادة المحاولة، ومنعُ الاستكمال يحبس
    # الباحثَ في نتيجةٍ تجاوزها المنتج.
    #
    # والفحصُ كان يحمل الحجّةَ المنقوضة، ولم يسقط لأنّه لم يُشغَّل على
    # قاعدةٍ حقيقية. فيُثبَّت القرارُ القائم حيث هو معلَّل.
    assert card["actions"]["can_mine"] is True, (
        "الاستكمالُ مُنع بعد «اكتمل بلا نتيجة» — والباحثُ يُحبس في نتيجةٍ "
        "تجاوزها المنتج")
