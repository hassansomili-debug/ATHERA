...
def test_28_the_response_is_sent_before_the_dependency_transaction_commits() -> None:
    ...
    **وقد أُغلق هذا الدَّينُ بنيويًّا** (RC-T1-H1، طور التقوية الأول): هذا
    الترتيبُ ما زال ترتيبَ FastAPI — وهو بعينه السببُ الذي جعل ملكيّةَ
    الإيداع تنتقل إلى `TransactionalRoute`، فتقع **داخل** معالج المسار
    قبل `await response(...)`. فالتبعيّةُ لم تعد تُودِع شيئًا
    (`owns_commit=False`)، ولم يبقَ في فكِّ الحزمة إلّا الرجوعُ والإغلاق.

    فهذا الفحصُ باقٍ **حارسًا للافتراض** لا إعلانًا لدَين: يقرأ الترتيبَ
    من الحزمة المُثبَّتة، فلو خرج الإرسالُ من متن الحزمة غدًا سقط —
    ووجب إعادةُ النظر في الغلاف (يصير زائدًا لا خاطئًا). والدعوى التي
    تحرس الصحّةَ اليوم هي
    `tests/test_at_rc_t1_h1_transaction_integrity.py`: قيدٌ مؤجَّلٌ حقيقيّ
    يُخفق عند الإيداع، والعميلُ يأخذ ٥٠٣ لا ٢٠١.
    """
    import inspect
