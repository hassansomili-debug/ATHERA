...
test.describe.serial("P0-T1 production acceptance", () => {
  test("A — upload to selection, with no step the researcher must perform", async ({ page }) => {
    ...
    await test.step("upload the synthetic thesis through the normal UI", async () => {
      ...
      // **رقمٌ علميٌّ واحدٌ في المستند كلِّه، ويُفحص قبل أن يُرفع.** فلو
      // تسلّل عددٌ ثانٍ لسقطت التشغيلةُ لاحقًا على قاعدةِ الأهليّة — وهي
      // قاعدةٌ صحيحة، فيُكشف الخللُ هنا في التجهيزة لا هناك في المنتج.
      //
      // **ولا يُعدّ إلّا رقمٌ قائمٌ بذاته.** الرقمُ داخل `PUBRIVA-T1-ACCEPT`
      // جزءٌ من معرّف لا قيمةٌ علمية، وقد أسقط `\d+` المجرَّدُ التشغيلةَ
      // عليه. والعلامةُ تبقى كما هي: هويّةُ التنظيف معلّقةٌ بها.
      const numbers =
        THESIS_TEXT.match(/(?<![A-Za-z0-9])\d+(?![A-Za-z0-9])/g) ?? [];
      expect(
        numbers,
